  ...
  if (flagEl && tooltipEl) {
    ...
    if (hasCoords) {
      flagEl.addEventListener("click", () => {
        const mapsUrl = `https://www.google.com/maps?q=${geo.latitude},${geo.longitude}&z=2`;
...
async function initStatsView(user: User | null) {
  ...
  const renderStats = async () => {
    ...
    const renderGeoFlag = (geo: { geo_country: string | null; geo_city: string | null; geo_region: string | null; geo_latitude: string | null; geo_longitude: string | null; geo_timezone: string | null }, id: string) => {
      ...
      const mapsUrl = hasCoords ? `https://www.google.com/maps?q=${geo.geo_latitude},${geo.geo_longitude}&z=2` : null;
      const tooltip = [
        geo.geo_city,
        geo.geo_region,
        geo.geo_country,
